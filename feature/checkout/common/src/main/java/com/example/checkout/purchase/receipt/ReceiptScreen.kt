...
fun ReceiptScreen(
    ...
    val uiModel: ReceiptUiModel = koinViewModel { parametersOf(purchaseReference) }
...
private fun PurchaseSummary(purchase: Purchase) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            purchase.paymentOrderId?.takeIf { it.isNotBlank() }?.let { SummaryRow("Pedido na adquirente", it) }
            purchase.authorizationCode?.let { SummaryRow("Autorização", it) }
            purchase.acquirerCode?.let { SummaryRow("Código da adquirente", it) }
